...
This lab explores an attack an Active Directory Kerberos Authentication. To be more precise - an attack that forges a Kerberos Ticket Granting Tickets that are used to authenticate the user with Kerberos. TGTs are used when requesting Ticket Granting Service \(TGS\) tickets, which means a forged TGT can get us any TGS ticket - hence it's golden.
